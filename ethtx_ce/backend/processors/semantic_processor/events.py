...
from ...models.decoded_model import DecodedEvent, TransactionMetadata

from ...processors.semantic_processor.helpers.utils import (
    create_transformation_context,
    semantically_decode_parameter,
    get_badge,
)
from ethtx_ce.semantics.protocols.anonymous import anonymous_events
from ethtx_ce.semantics.standards.erc20 import ERC20_EVENTS, ERC20_TRANSFORMATIONS
from ethtx_ce.semantics.standards.erc721 import ERC721_EVENTS, ERC721_TRANSFORMATIONS
...
class SemanticEventsDecoder(SemanticSubmoduleAbc):
    ...
    def decode(
        ...
        if isinstance(events, list):
            return (
                [self.decode_event(event, tx_metadata) for event in events]
                if events
                else []
            )

        return self.decode_event(events, tx_metadata)

    def decode_event(
        self, event: DecodedEvent, tx_metadata: TransactionMetadata
    ) -> DecodedEvent:
        ...
        if event.event_name != event.event_signature:
            ...
            calculated_event_signature = Web3.sha3(
