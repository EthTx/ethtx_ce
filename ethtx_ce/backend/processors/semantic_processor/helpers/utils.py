...
from ethtx_ce.backend.models.decoded_model import AddressInfo
from ethtx_ce.backend.processors.decoders.parameters import decode_function_parameters
from ethtx_ce.semantics.utilities.functions import add_utils_to_context

log = logging.getLogger(__name__)


def get_eth_price() -> float or None:
...
def semantically_decode_parameter(
    repository, parameter, indexed_name, transformations, context
):
    ...
    if parameter.type == "address" and not isinstance(parameter.value, AddressInfo):
        ...
        name = repository.get_address_label(
            context["__transaction__"].chain_id, address
        )
        badge = get_badge(
            address,
        ...
        for i, sub_parameter in enumerate(parameter.value):
            semantically_decode_parameter(
                repository, sub_parameter, f"__input{i}__", transformations, context
            )
