...
from ...models.decoded_model import DecodedCall, DecodedTransfer, AddressInfo
...
class ABITransfersDecoder(ABISubmoduleAbc):
    ...
    def decode(self, call: DecodedCall, events, token_proxies) -> List:
        ...
        def _transfers_calls(decoded_call):
            ...
            if decoded_call.sub_calls:
                for sub_call in decoded_call.sub_calls:
        ...
        for event in events:
            ...
            if event.event_name == "Transfer":
                ...
                from_name = self.repository.get_address_label(
                    event.chain_id, from_address
                )
                to_address = event.parameters[1].value
                to_name = self.repository.get_address_label(event.chain_id, to_address)

                standard = self.repository.get_standard(
                    event.chain_id, event.contract.address
                )

                if standard == "ERC20":

                    _, token_symbol, token_decimals = self.repository.get_token_data(
